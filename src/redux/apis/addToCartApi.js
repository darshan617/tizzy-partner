...
const addToCartApi = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    checkIsDomainAvailable: builder.query({
      query: ({ domain_name, provider_id, partner_id }) => {
        return {
          url: `/orders-check-domain?domain_name=${domain_name}&provider_id=${provider_id}&partner_id=${partner_id}`,
          method: "GET",
        };
      },
      providesTags: ["addToCart"],
    ...
    licenseAddToCart: builder.mutation({
      query: ({ body }) => {
        return {
          url: `/license-add-to-cart`,
          method: "POST",
          body: body,
        };
      },
    ...
    sendPurchaseRequest: builder.mutation({
      query: ({ body }) => {
        return {
