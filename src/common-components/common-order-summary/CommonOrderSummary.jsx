...
const CommonOrderSummary = () => {
  ...
  const total = isListCart
    ? cartDetails.reduce((sum, item) => {
        ...
        const licenses = Math.max(1, Number(item?.licenses) || 1);

        return sum + unitPrice * licenses;
      }, 0)
    : (Number(pricePerUser) || 0) * (Number(lisceneCounter) || 0);
    ...
    try {
      const res = await licensesAddToCart({
        body: {
          ...
          plan_id: plan?.id || plan?.plan_id || router?.query?.planId,
